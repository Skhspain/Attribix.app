...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const nonceCookieMatch = cookieHeader.match(/(?:^|;\s*)google_oauth_nonce=([^;]+)/);
  const storedNonce = nonceCookieMatch ? nonceCookieMatch[1] : null;
  const stateNonce = state?.nonce;

  if (!storedNonce || !stateNonce || storedNonce !== stateNonce) {
    console.warn(`[google.callback] Nonce mismatch for shop=${shop} — possible CSRF attempt`);
    return errorPage("OAuth state mismatch. Please try connecting again.");
  }
